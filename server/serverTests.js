...
async function tests(){
    ...
    const user = await fetchDefaultReturn(url+'/user/read/login',{ // GET USER INFO GIVEN LOGIN
        method:'POST',
        headers: {
            'Content-Type': 'application/json',
        },
    ...
    const userIdReq = await fetchDefaultReturn(url + `/user/read/id/${user._id}`) // GET USER INFO GIVEN ID
    .then(res=>res).catch(err => err);
    ...
    const courseReq = await fetchDefaultReturn(url+`/course/read/${user.courses[0]}`).then(res=>res).catch(err => err); //GET COURSE INFO USER IS IN

    console.log(courseReq);

    const addCourseReq = await fetchDefaultReturn(url + '/user/update/addCourse', { //ADD THIS COURSE TO THE USER (purposefully fails)
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
    ...
    const groupsBeforeCommit = user.groups.length;
    ...
    const groupsAfterCommit = user.groups.length;

    console.log("# of user's groups after creating new one:\n" + user.groups.length);

    if (groupsAfterCommit > groupsBeforeCommit){

        const group = await fetchDefaultReturn(url+`/group/read/${user.groups[user.groups.length-1]}`).then(res=>res).catch(err => err);//READ GROUP INFO FROM ID

        console.log("Created Group ID:\n" + group._id);

        const addGroupReq = await fetchDefaultReturn(url+'/user/update/addGroup', {//ADD GROUP TO USER (does nothing)
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        const addMemberReq = await fetchDefaultReturn(url+'/group/update/addUser', { //ADD USER TO GROUP (does nothing)
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        const searchGroupReq = await fetchDefaultReturn(url+'/group/search', { //SERCH FOR GROUPS IN COURSE
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        const deleteGroupReq = await fetchDefaultReturn(url+'/group/delete', { //DELETE GROUP
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        const findGroupReq = await fetchDefaultReturn(url+`/group/read/${group._id}`).then(res=>res).catch(err => err);
